...
from models.database import get_supabase_client  # unified import
...
async def get_gameplay_feedback(
    ...
    try:
        ...
        if "gameplay_analysis" not in analysis_data or not analysis_data["gameplay_analysis"]:
            raise HTTPException(status_code=404, detail="Gameplay analysis not available for this video")

        return GameplayAnalysis(**analysis_data["gameplay_analysis"])

    except Exception as e:
        logger.error(f"Get gameplay feedback error: {str(e)}")
        raise HTTPException(status_code=500, detail="Failed to retrieve gameplay feedback")
...
async def get_athlete_gameplay_feedback(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="Failed to retrieve athlete gameplay feedback")
